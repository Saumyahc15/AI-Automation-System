...
import { Mail, LockKeyhole, Github, Chrome, Sparkles } from 'lucide-react'
...
export default function LoginPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const result = await loginMutation.mutateAsync(formData) as any
      ...
      // Store avatar from backend (persists across sessions)
  ...
  return (
    <div className="relative flex min-h-screen items-center justify-start overflow-hidden px-4 py-10 text-white">
      <LoginBackground />
      <CursorFollowers loginState={loginState} />
        ...
        <form onSubmit={handleSubmit} className="space-y-5">
          ...
          <motion.div variants={itemVariants} className="flex gap-3">
            <motion.button
              ...
              whileTap={{ scale: 0.95 }}
              className="flex w-1/2 items-center justify-center gap-2 rounded-2xl bg-white/5 px-4 py-3 text-sm ring-1 ring-white/10 hover:bg-white/10 transition-all"
            >
