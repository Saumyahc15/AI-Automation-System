...
def seed_for_user(user_id: int, db):
    print(f"Seeding database for user_id={user_id}...")
    ...
    for s in suppliers_data:
        supplier = Supplier(**s, user_id=user_id)
        db.add(supplier)
    ...
    for i, (name, category, stock, price, threshold) in enumerate(products_data):
        ...
        p = Product(name=name, sku=sku, category=category, stock=stock,
                    price=price, low_stock_threshold=threshold, supplier_id=supplier.id, user_id=user_id)
        db.add(p)
        products.append(p)
    db.flush()
    ...
    for wf in sample_workflows:
        db.add(Workflow(**wf, user_id=user_id))

    db.commit()
    print(f"Data seeded for user_id={user_id}")
